...
async def list_tenant_templates(
    ...
    if db is not None:
        ...
        rows = await supabase_client.select(
            "templates",
            "id,tenant_id,name,language,subject,content,version,is_active,notification_type,created_at,channel_id",
            filters=filters,
        )
    ...
    global_count = sum(1 for t in serialized_templates if t["tenant_id"] is None)
    tenant_count = sum(1 for t in serialized_templates if t["tenant_id"] == tenant.id)
...
async def clone_global_template(
    ...
    tenant_template = Template(
        id=f"{tenant.id}_{name}_{global_template.channel}_{uuid.uuid4().hex[:8]}",
        tenant_id=tenant.id,
        name=name,
