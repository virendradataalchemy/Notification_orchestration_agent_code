...
export default function AnalyticsPage({
  ...
  const clientName = initialClientName || context?.resolved?.name || "";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await fetchJson<OverviewResponse>(`/api/client-dashboard/client/${clientId}/overview`, undefined, 5 * 60 * 1000);
        if (!active) return;
        setOverview(data);
