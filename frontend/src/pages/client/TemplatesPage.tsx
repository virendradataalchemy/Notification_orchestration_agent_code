...
export default function TemplatesPage({
  ...
  const [clientName, setClientName] = useState<string>(initialName);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const queryParams = new URLSearchParams({ include_global: String(includeGlobal) });
        if (channelFilter) queryParams.set("channel", channelFilter);
        const data = await fetchJson<TemplatesResponse>(`/api/v1/client/templates?${queryParams.toString()}`, {
            ...
            <article key={String(template.id)} className="rounded-[28px] border border-slate-200 bg-white/80 p-6 shadow-sm">
              <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex gap-3">
                  <Link
                    to={clientTemplateEditUrl(clientPath, template.id)}
